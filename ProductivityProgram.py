    ...
    # Inputs data from z days ago, updates data, weeklyData, aggregate, leisurepoints
        ...
        if y2 in variables.keys():
            ...
            if y2!= "l":
                ...
                leisurePoints += value2*multipliers[y2]*modifier
...
for observation in range(len(recentData)):
    time = recentData["Length"][observation]
    item = recentData["Item"][observation]
    date = recentData["Date"][observation]
    date = str(date.date())
    if item != "Leisure":
...
ratio = workHours/(workHours + leisureHours)
fig,x = plt.subplots()
...
desktop.paste(bar2, (startWidth+width1 + margins + barMargins + width3+1, 
                     startHeight))
desktop.paste(bar4, (startWidth+width1 + margins + barMargins*2 + width3*2,
                     startHeight))
desktop.paste(bar3, (startWidth+width1 + margins + barMargins*3 + width3*3 -2,
                     startHeight))
